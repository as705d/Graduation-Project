...
def Number():
	...
	#이미지 로드
	data = sorted(glob.glob('../images/2020_exceptzero/*'), key=str.lower)
	...
	#트레이닝 셋과 테스트 셋으로 분류
	#X_train, X_test, Y_train, Y_test = train_test_split(train_images, label_one, test_size = 0.01, random_state=33)


	#데이터 차원 설정
	#test_X = X_test.reshape(-1, 16384)
	#train_X = train_images.reshape(-1, 50176)
	test_X = train_images.reshape(-1, 50176)
	
	
	#데이터 저장
	#np.save('../results/train_X_2020.npy', train_X)
	np.save('../results/test_X_2020.npy', test_X)
	#np.save('../results/Y_train_2020.npy', label_one)
	np.save('../results/Y_test_2020.npy', label_one)
